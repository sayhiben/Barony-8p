		...
		&& players[player]->bControlEnabled && !gamePaused )
	{
		...
		if ( item )
		{
				...
				&& itemCategory(item) == SPELLBOOK && players[player] && players[player]->entity )
			{
				...
				if ( currentSpell && stats[player] )
				{
					int skillLVL = stats[player]->getModifiedProficiency(currentSpell->skillID) + statGetINT(stats[player], players[player]->entity);
					if ( stats[player]->getModifiedProficiency(currentSpell->skillID) >= 100 )
					{
						skillLVL = 100;
					}
					if ( skillLVL >= currentSpell->difficulty )
